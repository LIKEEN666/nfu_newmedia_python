...
## 输入：
用户输入日期，城市  见  [tempaltes/entry.html](tempaltes/entry.html)
## 输出：
...
後端伺服器web 响应：[find_the_weather_4web.py](find_the_weather_4web.py) 中 执行 了@app.route('/') 下的 entry_page()函数，以HTML模版 [tempaltes/entry.html](tempaltes/entry.html)会出现《欢迎使用天气查询工具》的HTML页面

前端浏览器收到web 响应：出现HTML页面有HTML表单的输入 input 类型(type) 为"text"，变数名称(name)为'city’和‘date’，详见HTML [tempaltes/entry.html](tempaltes/entry.html)
